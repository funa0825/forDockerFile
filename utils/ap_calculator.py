...
def parse_predictions(
    ...
    if config_dict["rotated_nms"]:
        ...
        #　デフォではここを通る
        #print("pred_corners",pred_corners_3d_upright_camera)
        #print("pred_Dim1",len(pred_corners_3d_upright_camera))
        #print("pred_Dim2",len(pred_corners_3d_upright_camera[0]))
        #print("pred_Dim3",len(pred_corners_3d_upright_camera[0][0]))
        #print("pred_Dim4",len(pred_corners_3d_upright_camera[0][0][0]))
        pred_mask = np.zeros((bsize, K))
        for i in range(bsize):
...
class APCalculator(object):
    ...
    def step(
        ...
        #print(scanname)
        #print("BPMC:",batch_pred_map_cls[0][0])
        BBoxs = []
        VColors =[]
        ClassLabels = []
        FaceVIDs = []
        BBoxScores = []
        count = 0
        for k in range(len(batch_pred_map_cls[0])):
            #if(batch_pred_map_cls[0][k][0] == 0):
            #    continue
            #print("Score:",batch_pred_map_cls[0][k][2])
            if batch_pred_map_cls[0][k][2] < 0.01:
                continue
            for i in range(len(batch_pred_map_cls[0][k][1])):
                #print("bboxC:",batch_pred_map_cls[0][k][1])
                BBoxs.append(batch_pred_map_cls[0][k][1][i])
                #print("class:",batch_pred_map_cls[0][k][0])
                #R = (batch_pred_map_cls[0][k][0])%3
                #G = (batch_pred_map_cls[0][k][0]/3)%3
                #B = (batch_pred_map_cls[0][k][0]/9)%3
                #VColors.append([R*100,G*100,B*100])
            ClassLabels.append(batch_pred_map_cls[0][k][0])
            BBoxScores.append(batch_pred_map_cls[0][k][2])
            ...
            #count += 1
        BBoxs,BBoxScores,ClassLabels = self.remove_duplicate_bbox(BBoxs,BBoxScores,ClassLabels)
        BBoxs2=[]
        count=0
        for k in range(len(BBoxScores)):
            for i in range(8):
                BBoxs2.append(BBoxs[k*8+i])
                R = (ClassLabels[k])%3
                G = (ClassLabels[k]/3)%3
                B = (ClassLabels[k]/9)%3
                VColors.append([R*100,G*100,B*100])
            ClassLabels.append(ClassLabels[k])
            FaceVIDs.append([1+(count*8),2+(count*8),3+(count*8),4+(count*8)])
            FaceVIDs.append([5+(count*8),6+(count*8),7+(count*8),8+(count*8)])
            FaceVIDs.append([1+(count*8),2+(count*8),6+(count*8),5+(count*8)])
            FaceVIDs.append([3+(count*8),4+(count*8),8+(count*8),7+(count*8)])
            ...
            FaceVIDs.append([2+(count*8),3+(count*8),7+(count*8),6+(count*8)])
            count += 1
        self.saveOBJ("./test_parsed"+str(count)+".obj",BBoxs2,FaceVIDs,VColors)
        self.count += 1
    ...
    def saveOBJ(self,filePath, vertices, faceVertIDs,vertexColors):
        ...
        f_out.close()
    def remove_duplicate_bbox(self,bbox,bboxscore,label):
        unique_bbox_index = []
        duplicate_bbox_index = []
        for i in range(len(bboxscore)):
            if i in duplicate_bbox_index:
                continue
            index_and_score = []
            index_and_score.append([i,bboxscore[i]])
            C1 = bbox[i*8 + 0]
            C2 = bbox[i*8 + 1]
            C3 = bbox[i*8 + 2]
            C4 = bbox[i*8 + 3]
            ...
            C8 = bbox[i*8 + 7]
            duplicated = False
            for j in range(len(bboxscore)):
                if i==j:
                    continue
                if j in duplicate_bbox_index:
                    continue
                CC1 = bbox[j*8 + 0]
                CC2 = bbox[j*8 + 1]
                CC3 = bbox[j*8 + 2]
                ...
                CC7 = bbox[j*8 + 6]
                CC8 = bbox[j*8 + 7]
                if all(C1==CC1) and all(C2==CC2) and all(C3==CC3) and all(C4 == CC4) and all(C5 == CC5) and all(C6 ==CC6) and all(C7==CC7) and all(C8 == CC8):
                    duplicated = True
                    duplicate_bbox_index.append(i)
                    duplicate_bbox_index.append(j)
                    index_and_score.append([j,bboxscore[j]])
                    #print("Find Duplicate:",i," and ",j)
            maxscore = np.argmax(index_and_score,axis=0)
            #print("maxScore",maxscore)
            maxIndex = index_and_score[maxscore[1]][0]
            #print("maxIndex",maxIndex)
            if duplicated:
                unique_bbox_index.append(maxIndex)
            else:
                unique_bbox_index.append(i)
        BBoxs = []
        BBoxScores = []
        Labels = []
        for i in range(len(unique_bbox_index)):
            #print("unique_bbox_index:",unique_bbox_index[i])
            BBoxs.append(bbox[unique_bbox_index[i]*8+0])
            BBoxs.append(bbox[unique_bbox_index[i]*8+1])
            BBoxs.append(bbox[unique_bbox_index[i]*8+2])
            BBoxs.append(bbox[unique_bbox_index[i]*8+3])
            BBoxs.append(bbox[unique_bbox_index[i]*8+4])
            BBoxs.append(bbox[unique_bbox_index[i]*8+5])
            BBoxs.append(bbox[unique_bbox_index[i]*8+6])
            BBoxs.append(bbox[unique_bbox_index[i]*8+7])
            BBoxScores.append(bboxscore[unique_bbox_index[i]])
            Labels.append(label[unique_bbox_index[i]])
        return BBoxs,BBoxScores,Labels
